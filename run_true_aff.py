...
parser.add_argument("--dataset_type", default = 'true_aff', type=str)
...
def train_generator(args, generator, oracle, tokenizer, dataset, global_weight):
    ...
    for it in tqdm(range(args.gen_num_iterations)):
        ...
        if it % 100 == 1:
            ...
            if args.saving:
                if args.save_base:
                    torch.save(generator.state_dict(), './model_parameters/true_aff_base_rep:{}.pt'.format(args.seed))
                else:
                    torch.save(generator.state_dict(), './model_parameters/true_aff_generator_sol:{},aff:{},gw:{},beta:{},rep:{}.pt'.format(global_weight[1],global_weight[2],global_weight[3],global_weight[4],args.seed))
...
def sample_batch(args, rollout_worker, generator, current_dataset, oracle, global_weight):
    ...
    sample_n = 2560
    #args.gen_data_sample_per_step = 64
    ...
    print(scores)
    if args.log_results:
        with open('./lib/dataset/gen_seqs/gflownet/true_aff/true_aff_gflow_sol:{}_aff:{}_global:{}_beta:{}_rep:{}.txt'.format(global_weight[1],global_weight[2],global_weight[3],global_weight[4],args.seed),'w') as f:
            for s in seqs:
                f.write('{}\n'.format(s))
        np.save('./lib/dataset/gen_seqs/gflownet/true_aff/true_aff_gflow_scores_sol:{}_aff:{}_gw:{}_beta:{}_rep:{}.npy'.format(global_weight[1],global_weight[2],global_weight[3],global_weight[4],args.seed),scores)
...
def train(args, oracle, dataset, global_weight):
    ...
    for round in range(args.num_rounds):
        ...
        if args.loading:
            if args.load_base:
                generator.load_state_dict(torch.load('./model_parameters/true_aff_base_rep:{}.pt'.format(args.seed),map_location=args.device))
            else:
                generator.load_state_dict(torch.load('./model_parameters/true_aff_generator_sol:{},aff:{},gw:{},beta:{},rep:{}.pt'.format(global_weight[1],global_weight[2],global_weight[3],global_weight[4],args.seed),map_location=args.device))
                spearman_mcmc_prior = np.load('./losses/true_aff_spearmanr_mcmc_sol:{}_aff:{}_gw:{}_beta:{}_rep:{}.npy'.format(global_weight[1],global_weight[2],global_weight[3],global_weight[4],args.seed))
        ...
        if args.saving:
            if args.save_base:
                ...
                torch.save(generator.state_dict(), './model_parameters/true_aff_generator_sol:{},aff:{},gw:{},beta:{},rep:{}.pt'.format(global_weight[1],global_weight[2],global_weight[3],global_weight[4],args.seed))
                np.save('./losses/true_aff_spearmanr_mcmc_sol:{}_aff:{}_gw:{}_beta:{}_rep:{}.npy'.format(global_weight[1],global_weight[2],global_weight[3],global_weight[4],args.seed),np.array(spearmanr_mcmc))
